...
import { TagIcon, DocumentTextIcon, CalendarIcon, ChartBarIcon } from '@heroicons/react/24/outline';
...
const TagStats: React.FC<TagStatsProps> = ({ tag, posts, className = '' }) => {
  ...
  const relatedTags = [...new Set(
    posts.flatMap(post => post.tags.filter(t => t !== tag.name))
  )].slice(0, 5);
    ...
    <div className={`bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 ${className}`}>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
          ...
          <div className="text-2xl font-bold text-blue-600 dark:text-blue-400">{totalPosts}</div>
        ...
        <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
          ...
          <div className="text-2xl font-bold text-green-600 dark:text-green-400">{averageReadingTime}</div>
          <div className="text-sm text-gray-600 dark:text-gray-400">平均阅读时间</div>
        </div>
      </div>
      ...
      {latestPost && (
        <div className="mb-6">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">最新文章</h4>
          <Link
      ...
      {categories.length > 0 && (
        <div className="mb-6">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">相关分类</h4>
      ...
      {relatedTags.length > 0 && (
        <div className="mb-6">
          <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">相关标签</h4>
          <div className="flex flex-wrap gap-2">
      ...
      {recentMonths.length > 0 && (
        <div>
          <div className="flex items-center mb-3">
            ...
            <h4 className="text-sm font-medium text-gray-700 dark:text-gray-300">月度发布趋势</h4>
          </div>
