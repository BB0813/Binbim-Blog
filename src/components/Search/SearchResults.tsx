  ...
  if (posts.length === 0) {
    return (
      <div className={className}>
        <Empty
          title="没有找到相关文章"
          description={query ? `没有找到包含 "${query}" 的文章` : '尝试使用不同的关键词'}
          action={
    ...
    <div className={className}>
      ...
      {(total !== undefined || searchTime !== undefined) && (
        <div className="mb-6 text-sm text-gray-600 dark:text-gray-400">
          {total !== undefined && (
            <span>
              找到 <span className="font-medium text-gray-900 dark:text-white">{total}</span> 个结果
            </span>
          )}
    ...
    `}>
      <div className="flex items-start justify-between">
        <div className="flex-1">
          ...
          {/* 文章元信息 */}
          <div className={`flex items-center text-sm text-gray-500 dark:text-gray-400 ${compact ? 'mb-2' : 'mb-3'}`}>
