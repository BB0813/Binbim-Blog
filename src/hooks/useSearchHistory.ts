...
export function useSearchHistory(config: UseSearchHistoryConfig = {}) {
  ...
  // 删除特定记录
  const removeSearchRecord = useCallback(
    (id: string) => {
      setHistory(prevHistory => {
  ...
  const getSearchStats = useCallback(() => {
    ...
    const averageResultCount = history
      ...
      .reduce(
        (sum, item, _, arr) => sum + (item.resultCount || 0) / arr.length,
        0
      );
    ...
    return {
      ...
      searchFrequency:
        totalSearches > 0 && oldestSearch && newestSearch
          ? totalSearches /
            Math.max(1, (newestSearch - oldestSearch) / (1000 * 60 * 60 * 24)) // 每天搜索次数
          : 0,
    };
  }, [history]);
