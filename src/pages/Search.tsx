...
const Search: React.FC = () => {
  ...
  // 过滤结果
  const filteredResults = search.results?.posts.filter(post => {
          ...
          <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="text-sm text-gray-600 dark:text-gray-400">
                {search.loading ? (
                  ...
                  <span>
                    找到 <span className="font-medium text-gray-900 dark:text-white">{filteredResults.length}</span> 个结果
                    {search.results.searchTime && (
                      <span className="ml-2">({search.results.searchTime.toFixed(2)}ms)</span>
                    )}
                  </span>
                ) : null}
              </div>
              
              {search.results && (availableCategories.length > 0 || availableTags.length > 0) && (
                <button
            ...
            {search.loading ? (
              ...
              <div className="text-center py-12">
                <div className="text-red-600 dark:text-red-400 mb-2">{search.error}</div>
                <button
